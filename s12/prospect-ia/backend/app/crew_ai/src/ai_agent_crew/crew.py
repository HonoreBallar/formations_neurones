...
class AiAgentCrew():
    ...
    def build(self, config: dict):
        """Builds the AiAgentCrew crew"""
        self.agents_config = load_agents_config(config)
        self.tasks_config = load_tasks_config(config)


    def get_agents(self) -> List[Agent]:
        return [
            Agent(config=self.agents_config['market_researcher'], tools=[...]),
            Agent(config=self.agents_config['prospecting_specialist']),
            Agent(config=self.agents_config['content_writer'])
        ]
